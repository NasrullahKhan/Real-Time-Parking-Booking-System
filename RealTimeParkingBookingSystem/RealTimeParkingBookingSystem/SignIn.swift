...
class SignIn: UIViewController, ValidationDelegate, UITextFieldDelegate  {
    ...
    func validationSuccessful() {
        ...
        Auth.login(email: self.emailTextField.text!, password: self.passwordTextField.text!) { (user, error) in
            ...
            if error == nil {
                let type = User.sharedUser.value!.userType!
                
                switch type {
                case .user:
                    self.performSegue(withIdentifier: "goToUserMainController", sender: nil)
                case .admin:
                    AdminServices.getAllUSers(comletion: { (error) in })
                    self.performSegue(withIdentifier: "goToAllUsersController", sender: nil)
                }
